...
class QifComission {
    ...
    static QifComission compute(Transaction transaction) {
        ...
        double charges = transaction.chargesProperty().getValue() == null ? 0d : transaction.chargesProperty().getValue().doubleValue();
        ...
        if (transaction.financeTaxProperty().getValue() != null) {
            commission.kapitalertragsteuer = transaction.financeTaxProperty().getValue().doubleValue();
        }
        if (transaction.solidarityTaxProperty().getValue() != null) {
            commission.solidaritaetsuzschlag = transaction.solidarityTaxProperty().getValue().doubleValue();
        }
        return commission;
