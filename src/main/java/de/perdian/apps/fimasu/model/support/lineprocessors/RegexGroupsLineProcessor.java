...
public class RegexGroupsLineProcessor extends RegexLineProcessor {
    ...
    public RegexGroupsLineProcessor setNumber(RegexGroupsLookup groupLookup, WritableValue<Number> groupProperty, NumberFormat numberFormat) {
        return this.setNumber(groupLookup, groupProperty, numberFormat, 1d);
    }

    public RegexGroupsLineProcessor setNumber(RegexGroupsLookup groupLookup, WritableValue<Number> groupProperty, NumberFormat numberFormat, double numberSign) {
        return this.set(groupLookup, groupProperty, value -> {
            try {
                return StringUtils.isEmpty(value) ? null : (numberFormat.parse(value).doubleValue() * numberSign);
    ...
    public RegexGroupsLineProcessor addNumber(RegexGroupsLookup groupLookup, WritableValue<Number> groupProperty, NumberFormat numberFormat, double numberSign) {
        return this.set(groupLookup, groupProperty, value -> {
            try {
                ...
                if (existingValue == null && newValue == null) {
                    ...
                    return (existingValue == null ? 0d : existingValue.doubleValue()) + (newValue == null ? 0 : newValue.doubleValue() * numberSign);
                }
            } catch (ParseException e) {
                throw new IllegalArgumentException("Invalid numberic value: " + value, e);
            }
