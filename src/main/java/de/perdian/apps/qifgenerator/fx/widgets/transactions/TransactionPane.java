...
class TransactionPane extends VBox {
    ...
    private Pane createSecondLinePane(TransactionComponentBuilder componentBuilder, Transaction transaction, ObservableList<Transaction> transactions) {
        ...
        secondLinePane.add(componentBuilder.createLabel("Market currency / price"), 3, 0, 2, 1);
        secondLinePane.add(componentBuilder.createTextField(transaction.getMarketCurrency()).width(50d).get(), 3, 1, 1, 1);
        secondLinePane.add(componentBuilder.createTextField(transaction.getMarketPrice(), new DoubleStringConverter("0.00000")).width(90d).get(), 4, 1, 1, 1);
    ...
    private Pane createThirdLinePane(TransactionComponentBuilder componentBuilder, Transaction transaction, ObservableList<Transaction> transactions) {
        ...
        thirdLinePane.add(componentBuilder.createTextField(transaction.getCharges(), new DoubleStringConverter("0.00")).width(160d).get(), 0, 1, 1, 1);
